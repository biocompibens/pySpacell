...
class Spacell(_neighborhood_matrix.NeighborhoodMatrixComputation, 
    ...
    def __init__(self, 
        ...
        names_for_index = ['feature', \
                            'neighborhood_matrix_type', 'neighborhood_p0', 'neighborhood_p1',  'neighborhood_nb_iterations',\
    ...
    def get_neighborhood_matrix(self, 
        ...
        try:
            if self._debug:
              ...
              (self.neighborhood_matrix_df['nb_iterations'] == iterations)]['neighborhood_matrix'])

            w = self.neighborhood_matrix_df[(self.neighborhood_matrix_df['type'] == neighborhood_matrix_type) &
              (self.neighborhood_matrix_df['p0'] == neighborhood_p0) &
              (self.neighborhood_matrix_df['p1'] == neighborhood_p1) &
              (self.neighborhood_matrix_df['nb_iterations'] == iterations)]['neighborhood_matrix'].values[0]
        except:
            raise ValueError("WARNING - neighborhood matrix not found")
    ...
    def get_suffix(self, 
        ...
        if neighborhood_matrix_type == 'radius':
            ...
            raise ValueError("in self.get_suffix, neighborhood_matrix_type {} not understood".format(neighborhood_matrix_type))
    ...
    def correlogram(self,
        ...
        neighborhood_min_p0, neighborhood_max_p1, iterations = self._check_neighborhood_matrix_parameters(neighborhood_matrix_type, neighborhood_min_p0, neighborhood_max_p1, **kwargs)
        ...
        for index in range(len(seq_points_x)-1):
            self.compute_per_image_analysis(feature_columns, method, neighborhood_matrix_type, seq_points_x[index], seq_points_x[index+1], **kwargs)
